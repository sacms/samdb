...
char *sacms(const char *id, const char *accessToken) {
    CURL *curl;
    CURLcode res;
    ...
    if (curl) {
        char url[256];
        snprintf(url, sizeof(url), "https://samdb.eu.org/api/GDfile?id=%s", id);
...
int main(void) {
    ...
    char *response = sacms("1KZ8VW_Pmy0Mbtcc5H_dczFEI3lc4zI5O", accessToken);
    if (response) {
        printf("%s\n", response);
        free(response);
    }
